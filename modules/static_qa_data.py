...
永見実行委員長（ミニ四駆大会・主催運営）向けの静的Q&Aとサジェスチョン。
...
STATIC_QA_JA: Dict[str, str] = {
    ...
    # --- 会場ルール（京セラみなとみらい） ---
    "コンセントの使用": "結論、NGです。会場でのコンセント使用は禁止されています。充電が必要な機器は事前にフル充電しておいてください。[EMOTION:neutral]",
    "喫煙所の場所": "結論、2Fエレベーター横に喫煙所があります。所定の場所以外での喫煙はご遠慮ください。[EMOTION:neutral]",
    "飲食・ゴミ分別": "結論、飲食は可能です。ただし、会場ビルのゴミ分別ルールが厳しいため、分別にはご注意ください。[EMOTION:neutral]",
    "金属パーツ・電池のゴミ処理": "結論、各自お持ち帰りください。金属パーツや電池などは産業廃棄物に該当するため、会場では処分できません。[EMOTION:neutral]",
    "練習走行の時間": "結論、10時〜12時です。練習走行はこの時間帯のみ行えます。時間厳守でお願いします。[EMOTION:neutral]",
    "レース形式・ポイント制": "結論、5レーンコースのレースを5回、ドラッグレースを3回行い、ポイントを競います。参加全チームの上位5チームが決勝へ進出します（各リーグ上位ではなく全体上位）。ポイント配分は、5レーンコースレース：勝利3PT・完走1PT・コースアウト0PT。ドラッグレース：勝利のみ3PT・2位以下およびコースアウトは0PTです。[EMOTION:neutraltalking]",
    "予選・決勝の電池ルール": "結論、予選は規定の電池でレースを行います。決勝のみアルカリ電池が支給されます。[EMOTION:neutral]",
    "モーター（当日現地購入のみ）": "結論、当日現地購入のトルクチューンモーターのみ使用可能です。モーターには剥がせない印が付きますのでご了承ください。事前購入品の持ち込みは不可です。[EMOTION:neutral]",
    # --- 解決確認 ---
...
def get_suggestions_for_phase(
    ...
    return main[:_MAX_MAIN_SUGGESTIONS] + [FREE_INPUT_HINT_JA, BACK_TO_MAIN_JA]
